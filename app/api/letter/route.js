...
Your job: Generate a professional engagement letter based on the user-supplied questionnaire data and valuation assessment output (enclosed between <DATA> … </DATA>) using the exact template structure provided below. Start IMMEDIATELY with letter content (no introductory text)
----------------------------
ENGAGEMENT LETTER TEMPLATE
----------------------------
Use this exact template structure and wording, replacing only the bracketed placeholders with appropriate information from the user data:
...
1. Introduction
This letter confirms that we, FVA Advisory Pte Ltd ("FVA" or "we") have been retained by you, [COMPANY_NAME] ("Client" or "you"), to provide the services ("the Services") described below. This letter (the "Contract") form the terms of this engagement.

2. Background and Purpose of the Valuation
[The Target / Target Group] is [BRIEF_DESCRIPTION]. [PURPOSE_CONTEXT_PARAGRAPH]
In this regard, the Client intends to engage FVA to estimate the Market Value of [OWNERSHIP_STAKE] in the [Target / Target Group] in relation to [PURPOSE_REASON].
As discussed and agreed with you, our valuation will be performed as of [VALUATION_DATE] and in accordance with the [required IFRS [if financial reporting purposes]] and International Valuation Standards ("IVS").
...
Preparation of a valuation report in compliance with the requirements of IVS ("Deliverable");
...
Any changes in Valuation Date, will result in substantial rework and hence result in additional fee
We will issue our invoice upon submission of the draft Deliverable.
Payment is due within 14 days from the date of invoice issuance. No penalties will be applied for late payment; however, delayed payment may result in corresponding delays in the submission of the final valuation report.

6. Restriction on Use of Our Deliverable
...
I have read the contract terms set out in the engagement letter dated [CURRENT_DATE]. I accept those contract terms on behalf of [COMPANY_NAME] and represent that I am authorized to do so.
...
**Replacement Rules:**
- [COMPANY_NAME] = Extract from user data
- [COMPANY_ADDRESS] = Extract from user data
- [CONTACT_PERSON] = Extract from user data
- [CONTACT_TITLE] = Extract from user data
- [CURRENT_DATE] = Use current date
- [BRIEF_DESCRIPTION] = Use target_description from questionnaire
- [PURPOSE_CONTEXT_PARAGRAPH] = Build context paragraph based on purpose
- [OWNERSHIP_STAKE] = Use ownership_percentage from questionnaire
- [TARGET_NAME] = Use company name
- [PURPOSE_REASON] = Use purpose from questionnaire
